...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    ...
    //float h_value = abs(cur_node.x-this->end_node->x) + abs(cur_node.y - this->end_node->y);
    return h_value;
}
...
RouteModel::Node *RoutePlanner::NextNode() {
    ...
    float lowest_sum = std::numeric_limits<float>::max();
    for (RouteModel::Node * node : this->open_list) {
        float sum = node->g_value + node->h_value;
        if (sum < lowest_sum) {
            lowest_sum = sum;
            lowest_node = node;
        }
    }
    open_list.erase(std::remove(open_list.begin(), open_list.end(), lowest_node), open_list.end());
    return lowest_node;
}
...
void RoutePlanner::AStarSearch() {
    ...
    while(open_list.size()>0) {
        ...
        if (current_node == end_node && current_node->visited) {
